    ...
    def getNextState(self, state, action):
        ...
        if (nextx,nexty) in self.corners:
            if nextx==1 and nexty==1:
                cornersSituation=[True,corners[1],corners[2],corners[3]]
            elif nextx == 1 and nexty == self.top:
                cornersSituation = [corners[0], True, corners[2], corners[3]]
            elif nextx==self.right and nexty==1:
                cornersSituation=[corners[0],corners[1],True,corners[3]]
            elif nextx==self.right and nexty==self.top:
                cornersSituation=[corners[0],corners[1],corners[2],True]
            succ=((nextx,nexty),cornersSituation)
        else:
            succ = ((nextx, nexty), corners)
        return succ
        util.raiseNotDefined()
...
def cornersHeuristic(state, problem):
    ...
    distCorners=[]
    for c in corners:
      distCorners.append(util.manhattanDistance(c,pos))

    maxim=max(distCorners)
    """
    currPos=state[0]
    cornersList=[]
    sumH=0
    ...
    for s in state[1]:
        if s==False:
            cornersList.append(corners[index])
        index=index+1
    while len(cornersList):
     distanceToCorner = []
     for c in cornersList:
        dist=util.manhattanDistance(currPos,c)
        distanceToCorner.append((dist,c))
     currDistance, currCorner=min(distanceToCorner)
     sumH = sumH + currDistance
     currPos = currCorner
     cornersList.remove(currCorner)

    return sumH# Default to trivial solution
...
def foodHeuristic(state, problem):
    ...
    for f in foodCoordinates:
        minList.append((util.manhattanDistance(f,position),f))
    if len(minList)==0:
        return 0
    dist,foodPos=min(minList)
    for f in foodCoordinates:
        maxList.append(util.manhattanDistance(f,foodPos))
    if len(maxList) == 0:
        return dist
    distM = max(maxList)
    return distM+dist
